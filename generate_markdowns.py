...
parser.add_argument('-l','--list', nargs='+',
                    help='<Required> List of app e.g ocean.py provider aquarius', required=True)

parser.add_argument('-m','--modules', nargs='+', help='<optional> List of modules', required=False)
...
def generate_markdowns(app: str, path: str, output_dir: str, doc_ignore_path: str, modules):
    '''
    ...
    for i in tqdm(range(len(markdowns_to_generate))):
        ...
        with open(file_path, 'a') as fp:
            subprocess.Popen(['pydoc-markdown', '-I', path, '-m', module_path, config], stdout=fp)
...
if __name__ == '__main__':
    ...
    for repository_info in markdowns_to_be_generated:
        ...
        generate_markdowns(markdown_repo['app'], markdown_repo['path'], markdown_repo['output_dir'],
                            markdown_repo['docignore_file_path'], modules)
